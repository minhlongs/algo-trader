...
/** Minimal interface for BullMQ-style queues used as legacy transport. */
interface LegacyQueue {
  add(name: string, data?: unknown): Promise<{ finished(): Promise<unknown> }>;
}
...
export class ConnectionPoolManager {
  ...
  initPool(service: string, transport: { fetch: typeof fetch }): void {
    ...
    this._transport = this._transport || new Map<string, { fetch: typeof fetch; _legacyQueue?: LegacyQueue }>();
    this._transport.set(service, transport);
  }

  private _transport: Map<string, { fetch: typeof fetch; _legacyQueue?: LegacyQueue }> | null = null;
  ...
  async fetchWithPool(
    ...
    try {
      ...
      if (response && typeof response === 'object' && 'finished' in response && typeof (response as Record<string, unknown>).finished === 'function') {
  ...
  initQueue(service: string, queue: LegacyQueue): void {
    ...
    if (entry) {
      ...
      this._transport.set(service, { fetch: async () => new Response(), _legacyQueue: queue });
    }
  ...
  async enqueueRequest(
    ...
    return {
      finished: async () => {
        ...
        return { data: (data && typeof data === 'object' && 'data' in data ? (data as Record<string, unknown>).data : 'ok') as string };
      },
    };
  }
}
