...
describe('kalman-filter-tracker::KalmanFilter1D', () => {
  it('initializes with default state 0', () => {
    const kf = new KalmanFilter1D(0.01, 0.1);
    expect(kf.getState()).toBeCloseTo(0, 5);
  });
...
describe('kalman-filter-tracker::calcResidualZScore', () => {
  ...
  it('returns 0 for zero-variance history', () => {
    expect(calcResidualZScore(0.8, [0.7, 0.7, 0.7])).toBe(0);
  });
