...
vi.mock('../live-order-polling', () => ({ schedulePollFor: mockSchedulePollFor }));
vi.mock('../live-order-manager-terminal', () => ({ handleFillFor: mockHandleFillFor }));
vi.mock('../live-order-risk-gates', () => ({ runRiskGates: mockRunRiskGates }));
...
import { DEFAULT_MAX_ORDER_LIFETIME } from '../live-order-manager-types';
...
describe('LiveOrderManager', () => {
  ...
  describe('re-exports', () => {
    it('exports TokenBucketRateLimiter', async () => {
      const { TokenBucketRateLimiter } = await import('../live-order-manager');
    ...
    it('exports OrderState type (compiles)', async () => {
      ...
      const mod = await import('../live-order-manager');
