...
export async function handleTradeStart(opts: {
  ...
  if (isLive) {
    ...
    if (missing.length > 0) {
      ...
      logger.error('Set POLYMARKET_API_KEY, POLYMARKET_API_SECRET, POLYMARKET_PASSPHRASE, POLYMARKET_PRIVATE_KEY in your .env or use --mode=paper for simulation.');
      process.exit(1);
    }

    if (!opts.yes) {
      logger.info('⚠️  LIVE TRADING: This will use REAL USDC on Polymarket.');
      logger.info(`   Capital: $${capitalUsdc} | Strategy: ${opts.strategy}`);
      logger.info('   Risk gates: 2% max position, 5% daily loss, 10 max positions, 3-loss circuit breaker');
  ...
  logger.info(`Mode: ${modeLabel} | Capital: $${capitalUsdc} | Strategy: ${opts.strategy}`);
  ...
  logger.info(`Orchestrator running (${modeLabel}). Press Ctrl+C to stop.`);
  logger.info('Endgame scanner active — scanning every 30s for high-probability markets.\n');
