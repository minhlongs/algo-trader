 ...
 *    Centralized in forest/rate-limit/ (Phase 35 R2).
...
describe('Signal-ingest HMAC contract discipline — 50th edge (PENTACONTAGON milestone)', () => {
  ...
  it('Zod schema bounds: size [0,1], confidence [0,1], ttlSec [60, 86400]', () => {
    ...
    const ttlRe = new RegExp(
      `ttlSec\\s*:\\s*z\\.number\\(\\)\\.int\\(\\)\\.min\\(${TTL_MIN_SECONDS}\\)\\.max\\(${TTL_MAX_SECONDS}\\)`,
    );
    expect(
      ttlRe.test(src),
  ...
  it('rate-limit: requestsPerMin >= 1 (denial-of-wallet prevention)', () => {
    // Phase 35 R2: rate-limit config centralized in forest/rate-limit/
    // Actual DEFAULT_TIER_LIMITS.requestsPerMin = 10; TIER_RATE_LIMITS tiers vary.
    // Invariant: requestsPerMin must be set (> 0), not absent or 0 (= unlimited).
    const rateLimitSrc = readFileSync(RATE_LIMIT_FILE, 'utf8');
    const rpmMatch = /requestsPerMin\s*:\s*(\d[\d_]*)/.exec(rateLimitSrc);
    expect(rpmMatch, 'requestsPerMin not found in forest/rate-limit/ — rate limiter not configured').not.toBeNull();
    const rpm = rpmMatch ? Number(rpmMatch[1].replace(/_/g, '')) : 0;
    expect(
      rpm,
      `rate-limit requestsPerMin=${rpm} — must be > 0 (unlimited = 0 is insecure for ingest)`,
    ).toBeGreaterThan(0);
  });
  ...
  it('factory pattern: createSignalIngestRouter(store: SignalStore)', () => {
    expect(
      /export\s+function\s+createSignalIngestRouter\s*\(\s*store\s*:\s*SignalStore\s*\)/.test(
        src,
      ),
  ...
  it('composite: 10 axes hold simultaneously (HMAC security-contract coherence)', () => {
    ...
    const rateLimitSrc = readFileSync(RATE_LIMIT_FILE, 'utf8');
    const rpmMatch = /requestsPerMin\s*:\s*(\d[\d_]*)/.exec(rateLimitSrc);
    const rpmVal = rpmMatch ? Number(rpmMatch[1].replace(/_/g, '')) : 0;
    expect(rpmVal, 'rate-limit requestsPerMin must be > 0').toBeGreaterThan(0);
