...
describe('Express API-server security middleware discipline — 43rd edge (TRITETRACONTAGON)', () => {
  ...
  it('`rateLimit` imported AND applied to `/api` path scope', () => {
    ...
    const hasNewImport = src.includes("'../../forest/rate-limit/redis-rate-limiter'")
      || src.includes('"../../forest/rate-limit/redis-rate-limiter"');
    expect(hasOldImport || hasNewImport, 'rate-limit module not imported').toBe(true);
    const hasRateLimit = src.includes('rateLimitMiddleware') && (src.includes('rateLimitMiddleware()') || /rateLimitMiddleware\s*\(/.test(src));
