 ...
 * Target: 100% coverage for src/desk/signal/sse-signal-broadcaster.ts
 */

import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';
import { SseSignalBroadcaster, sseBroadcaster } from '../../../../src/desk/signal/sse-signal-broadcaster';
...
describe('SseSignalBroadcaster', () => {
  ...
  describe('broadcast', () => {
    ...
    it('logs debug message with signal id and connection count', () => {
      const { res: res1 } = createMockResponse();
      const { res: res2 } = createMockResponse();
      ...
      const signal = createMockSignal({ id: 'signal-debug-1' });
      broadcaster.broadcast(signal);

      // The logger is mocked, we can't easily check it without mocking logger
